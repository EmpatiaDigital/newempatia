  ...
  useEffect(() => {
    const fetchPosts = async (): Promise<void> => {
      try {
        ...
        const data   = await res.json() as PostsResponse | Publicacion[];
        const userId = localStorage.getItem('userId');

        const lista: Publicacion[] = Array.isArray(data)
          ? data
          : (data.posts ?? []);
      ...
      {publicaciones.length === 0 ? (
        ...
        <div className="grid-publicaciones">
          {publicaciones.map((item) => {
            const bg = item.portada ? `url(${item.portada})` : `url(${fondoImg.src})`;
