...
function ComprobanteModal({
  ...
  const estadoInfo = ESTADO_INFO[pago.estado] ?? {
    bg: "#f1f5f9",
    color: "#475569",
    label: pago.estado,
  };
...
function VistaPagos({ onVolver }: { onVolver: () => void }) {
  ...
  const handleAceptar = async (id: string) => {
    ...
    setPagos((prev) =>
      prev.map((p) => (p._id === id ? { ...p, estado: "aceptado" } : p)),
    );
    setPagoModal((prev) =>
      prev?._id === id ? { ...prev, estado: "aceptado" } : prev,
    );
    Swal.fire({
      icon: "success",
      title: "Pago aceptado",
      text: "La cuota quedó marcada como pagada.",
      timer: 2000,
  ...
  const handleRechazar = async (id: string) => {
    ...
    setPagos((prev) =>
      prev.map((p) =>
        p._id === id ? { ...p, estado: "rechazado", notaAdmin: nota ?? "" } : p,
      ),
    );
    setPagoModal((prev) =>
      prev?._id === id
        ? { ...prev, estado: "rechazado", notaAdmin: nota ?? "" }
        : prev,
    );
    Swal.fire({
      icon: "info",
      title: "Pago rechazado",
      text: "El socio podrá reenviar el comprobante.",
      timer: 2000,
      showConfirmButton: false,
    });
  };
        ...
        {loading ? (
          ...
          >
            {pagos.map((pago) => {
              const socio = pago.socioId;
              const nombreMes = NOMBRES_MESES[(pago.mes ?? 1) - 1];
              const ei = ESTADO_INFO[pago.estado] ?? {
                bg: "#f1f5f9",
                color: "#475569",
                label: pago.estado,
              };
...
export default function SuperAdminDashboard() {
  ...
  // ── Campanita: cantidad de comprobantes en revisión ──
  ...
  useEffect(() => {
    ...
    fetch("https://newempatiabackend.vercel.app/api/pagos?estado=comprobante_enviado", {
      ...
      .then((r) => r.json())
      .then((data: unknown) =>
        setPendientes(Array.isArray(data) ? data.length : 0),
      )
  ...
  const manejarCongelado = async (socio: Socio) => {
    const { value: formValues, isConfirmed } = await Swal.fire({
      ...
      preConfirm: () => {
        const tipo = (document.getElementById("swal-tipo") as HTMLSelectElement)
          .value;
        const cantidad = (
          document.getElementById("swal-tiempo") as HTMLInputElement
        ).value;
    ...
    try {
      ...
      Swal.fire(
        "Cuenta pausada",
        `${socio.nombre} fue congelado correctamente.`,
        "success",
      );
      setSocios((prev) =>
        prev.map((s) =>
          s._id === socio._id
            ? { ...s, active: false, freezeUntil: tiempoFinal.toISOString() }
            : s,
        ),
      );
    } catch {
  ...
  const manejarHabilitar = async (socio: Socio) => {
    ...
    try {
      ...
      Swal.fire(
        "Habilitado",
        `${socio.nombre} vuelve a estar activo.`,
        "success",
      );
      setSocios((prev) =>
        prev.map((s) =>
        ...
        <div style={{ width: "100%", maxWidth: "900px" }}>
          ...
          >
            <div>
              ...
              >
                Congelá o habilitá cuentas de socios
              </p>
            </div>
            <span
              style={{
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: "38px",
                        borderRadius: "50%",
                        background: socio.active ? "#dcfce7" : "#fee2e2",
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        fontSize: "13px",
                        fontWeight: 700,
                        flexShrink: 0,
                        color: socio.active ? "#16a34a" : "#dc2626",
                      }}
                    >
                      {socio.nombre[0]}
                      {socio.apellido[0]}
                    ...
                    <div>
                      ...
                      {!socio.active && socio.freezeUntil && (
                        ...
                        >
                          Hasta el{" "}
                          <strong>
                            {new Date(socio.freezeUntil).toLocaleDateString(
                              "es-AR",
                            )}
                          </strong>{" "}
                          a las{" "}
                          {new Date(socio.freezeUntil).toLocaleTimeString(
                            "es-AR",
  ...
  return (
    <div className="socio-dashboard-container">
      {/* CSS campanita — solo lo que es nuevo */}
      <style>{`
        .sa-btn-comprobantes {
          display: flex;
          align-items: center;
        ...
        .sa-btn-text { flex: 1; text-align: left; }
        .sa-badge {
          display: inline-flex;
          align-items: center;
          gap: 4px;
        ...
        .sa-badge--activo {
          background: #dc2626;
          color: #fff;
          animation: sa-pulso 1.5s ease-in-out infinite;
        }
        .sa-badge--vacio {
          background: #e5e7eb;
          color: #6b7280;
      ...
      <div className="cards-container">
        ...
        {/* Card: Comprobantes de pago ── CON CAMPANITA */}
        <div className="card-header-modern-complete">
          <div
            className="card-header"
          ...
          <div className="card-body">
            ...
            >
              Revisá los comprobantes enviados por los socios y aceptá o rechazá
              cada uno.
            </p>
            <button
              className="card-changes sa-btn-comprobantes"
              onClick={() => setVista("pagos")}
            >
              <span className="sa-btn-text">Ver comprobantes</span>
              <span
                className={
                  pendientes > 0
                    ? "sa-badge sa-badge--activo"
                    : "sa-badge sa-badge--vacio"
                }
              >
        ...
        {/* Card: Banner de estadísticas — movida aquí desde el modal */}
        <div className="card-header-modern-complete">
          <div
            className="card-header"
            style={{ background: "linear-gradient(135deg, #4a1d96, #6d28d9)" }}
          >
            <h1>Banner de Estadísticas</h1>
            <Image src={Logo} alt="Logo" className="card-avatar" />
          </div>
          <div className="card-body">
            <p
            ...
            >
              Editá el texto y las métricas del banner público.
            </p>
            <StatsBanner apiBase="https://newempatiabackend.vercel.app" />
          </div>
        </div>
