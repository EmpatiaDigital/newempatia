          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {pagos.map((pago) => {
              ...
              return (
                ...
                >
                  <div style={{ width: "42px", height: "42px", borderRadius: "50%", background: "#dbeafe", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "15px", fontWeight: 700, color: "#1d4ed8" }}>
                    {(socio?.nombre?.[0] ?? "?").toUpperCase()}
                  </div>
                  ...
                  <button
                    ...
                    style={{ padding: "8px 16px", background: "#1e293b", color: "#fff", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "13px", fontWeight: 600, whiteSpace: "nowrap" }}
                  >
                    🖼 Ver comprobante
                  </button>
                </div>
...
export default function SuperAdminDashboard() {
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    fetch("http://localhost:5000/api/pagos?estado=comprobante_enviado", {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((r) => r.json())
      .then((data: unknown) => setPendientes(Array.isArray(data) ? data.length : 0))
        ...
        <div className="card-header-modern-complete">
          ...
          <div className="card-body">
            ...
            >
              ...
              <span className={pendientes > 0 ? "sa-badge sa-badge--activo" : "sa-badge sa-badge--vacio"}>
